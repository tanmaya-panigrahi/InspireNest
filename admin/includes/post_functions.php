...
function getAllPosts()
{
    ...
    if (isset($_SESSION['user']['role'])) {
        ...
        if ($_SESSION['user']['role'] == "Admin") {
            ...
            // Handle other roles or unset role
            $sql = "SELECT * FROM posts WHERE 1=0"; // No posts will be returned
        }
    } else {
        // Handle case where user role is not set
        $sql = "SELECT * FROM posts WHERE 1=0"; // No posts will be returned
    }
...
function updatePost($request_values)
{
    ...
    if (count($errors) == 0) {
        ...
        if (mysqli_query($conn, $query)) { // if post updated successfully
            if (isset($topic_id)) {
                // delete existing relationship between post and topic
                $sql = "DELETE FROM post_topic WHERE post_id=$post_id";
                mysqli_query($conn, $sql);
                // create new relationship between post and topic
